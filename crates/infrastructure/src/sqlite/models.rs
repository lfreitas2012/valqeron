//! Row model newtypes for the SQLite backend.
//!
//! Each model wraps a domain value and implements [`FromRow`], keeping the
//! row → domain reconstitution in one declarative place (the rusqlite analogue
//! of sqlx's `FromRow` model structs). Repositories select into these models
//! and then unwrap the inner domain value.

use rusqlite::Row;
use valqeron_core::{Issuer, Versioned};

use crate::sqlite::mapping::{
    FromRow, column_datetime, column_issuer_id, column_opt_cnpj, column_opt_country_code,
    column_opt_lei, column_opt_name, column_status,
};
...
#[derive(Debug)]
pub struct IssuerRow(pub Versioned<Issuer>);

impl IssuerRow {
    /// Consume the row, yielding the versioned issuer it wraps.
    pub fn into_inner(self) -> Versioned<Issuer> {
        self.0
    }
}
