...
//!
//! Each function is a single, focused SQL operation: it prepares a (cached)
//! statement, binds parameters, and returns a raw rusqlite result — either a row
//! model ([`IssuerRow`]) or an affected-row count. This layer holds **all** the
//! SQL and nothing else: it applies no conflict/not-found policy and performs no
//! `anyhow` wrapping. Those concerns belong to
//! [`crate::sqlite::repository::SqliteIssuerRepository`], which composes these
//! functions.
//!
...
use crate::sqlite::mapping::{FromRow, status_as_str};
use crate::sqlite::models::IssuerRow;
...
pub fn insert(conn: &Connection, issuer: &Issuer) -> rusqlite::Result<usize> {
...
pub fn delete(conn: &Connection, id: &IssuerId, expected_version: u32) -> rusqlite::Result<usize> {
