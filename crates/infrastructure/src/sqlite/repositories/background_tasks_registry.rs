...
use valqeron_core::{
    BackgroundTaskDefinition, BackgroundTaskDefinitionSnapshot, BackgroundTaskName,
    BackgroundTasksRegistryRepository,
};

// ======================== MODEL ========================
#[derive(Debug)]
pub(crate) struct BackgroundTasksRegistryRow(pub Versioned<BackgroundTaskDefinitionSnapshot>);

impl BackgroundTasksRegistryRow {
    pub(crate) fn into_inner(self) -> Versioned<BackgroundTaskDefinitionSnapshot> {
        self.0
    }
}
...
impl FromRow for BackgroundTasksRegistryRow {
    fn from_row(row: &Row) -> rusqlite::Result<Self> {
        let snapshot = BackgroundTaskDefinitionSnapshot {
...
fn reconstitute_lazy(row: BackgroundTasksRegistryRow) -> Versioned<BackgroundTaskDefinition> {
    let Versioned { data, version } = row.into_inner();
    Versioned {
        data: BackgroundTaskDefinition::reconstitute(data),
        version,
    }
}
...
impl BackgroundTasksRegistryRepository for SqliteBackgroundTasksRepository {
    ...
    fn find_by_id(
        &self,
        id: &UniqueIdentifier,
    ) -> RepositoryResult<Option<Versioned<BackgroundTaskDefinition>>> {
