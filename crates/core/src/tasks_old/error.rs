...
pub enum TaskCategoryError {
    #[error("Invalid category. Must be one of: {categories:?}",
        categories = vec!["ENGINE_SYSTEM", "FINANCE_DATA_SYNC", "OTHER"]
    )]
    InvalidCategory,
}
...
pub enum TaskTriggerError {
    #[error("Invalid trigger kind. Must be one of: {kinds:?}",
        kinds = vec!["INTERVAL", "RECURRING", "SYNC"]
    )]
    InvalidTrigger,
}
...
pub enum LogPolicyError {
    #[error("Invalid log policy. Must be one of: {policies:?}",
        policies = vec!["ALL", "FAILURES_ONLY"]
    )]
    InvalidPolicy,
}
...
pub enum SyncOutcomeKindError {
    #[error("Invalid outcome. Must be one of: {kinds:?}",
        kinds = vec!["SYNCED", "NOT_READY", "FAILED"]
    )]
    InvalidKind,
}
