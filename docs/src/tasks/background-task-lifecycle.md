...
State-specific accessors: `BackgroundTask<Retrying>::last_error()`, `BackgroundTask<Success>::output()`,
`BackgroundTask<Failed>::error()`.
...
`TaskStatus` implements `Display`, formatting as lowercase state names (`"pending"`, `"running"`, `"retrying"`,
